...
- [x] 동일 상품에 여러명이 좋아요/싫어요를 요청해도, 좋아요 수가 정상 반영된다. *(like_count 원자적 UPDATE — 10명 동시 좋아요 E2E 통과)*
- [x] 동일 쿠폰으로 여러 기기에서 동시 주문해도, 쿠폰은 단 한번만 사용된다. *(조건부 UPDATE affected=0 → 실패, E2E 동시성 테스트 통과)*
- [x] 동일 상품에 여러 주문이 동시 요청되어도, 재고가 정상 차감된다. *(reserve 조건부 UPDATE — 재고5 10주문 오버셀 없음 E2E 통과)*
...
| 좋아요 카운터 | like_count Lost Update | **원자적 UPDATE** (`SET like_count = like_count ± 1`) | 인메모리 증감→원자 UPDATE 전환, 동시 좋아요 정확 반영 |
