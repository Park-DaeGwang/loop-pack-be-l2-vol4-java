...
| 전체 설정 검토 — 죽은 설정/템플릿 잔재 정리 | ✅ | 과제 마무리 검토에서 발견한 항목 일괄 수정. **(1) DemoKafkaConsumer 삭제** — 템플릿 데모 코드인데 `demo.internal.topic-v1`에 `NewTopic` 빈이 없어서, 브로커 auto-create를 끈 이후로는 로컬 기동 시 UNKNOWN_TOPIC_OR_PARTITION 경고가 무한 반복될 상황이었음(+`System.out.println` 사용 — 컨벤션 위반). **(2) `consumer.value-serializer` 오타** — 컨슈머엔 `value-deserializer`가 맞는 키라 Spring이 조용히 무시, 실제 런타임은 Boot 기본값 StringDeserializer로 동작 중이었음(그래서 `(String)` 캐스팅이 살아있던 것). 적힌 대로 ByteArrayDeserializer가 활성화됐다면 모든 컨슈머가 ClassCastException으로 죽었을 것 — 실제 동작과 일치하게 `value-deserializer: StringDeserializer`로 정정. **(3) `admin.properties.bootstrap.servers: kafka:9092` 삭제** — 테스트 로그의 AdminClientConfig 덤프로 실증: Boot 3.2+의 KafkaConnectionDetails가 admin bootstrap을 최상위 `bootstrap-servers`로 항상 덮어써서 이 값은 적용된 적 없음(적용됐다면 도커 내부 호스트명이라 로컬 토픽 생성이 깨졌을 값). **(4) `enable-auto-commit`** — `properties` 맵 아래 하이픈 키는 Kafka가 unknown config로 무시(spring-kafka가 manual ack일 때 강제 false라 무해했음), 올바른 위치 `spring.kafka.consumer.enable-auto-commit`으로 이동. **(5) Schema Registry 잔재**(`auto.register.schemas`/`use.latest.version`) 삭제 — 레지스트리 미사용. **(6) KafkaConfig 주석 `.DLT`→`-dlt` 정정. **(7) OutboxPublisher** `send().get()`에 10초 타임아웃 추가 — 무한 get()은 브로커 장애 시 delivery.timeout(2분)까지 스케줄러 스레드를 묶음. **(8) `addNotRetryableExceptions(JacksonException.class)`** — 깨진 JSON 같은 결정적 실패는 재시도(3회×~10초)가 무의미하므로 즉시 DLQ로 |
| **버그**: staleness 체크가 순서 무관 연산(view/sales)까지 적용됨 | ✅ | 체크리스트의 "최신 이벤트만 반영"을 사용자가 되짚어보다가 발견 — `lastEventAt` 기반 stale 체크는 like/unlike처럼 서로 역연산이라 순서가 결과에 영향을 주는 연산에만 필요한데, 실제로는 `ProductMetricsModel`의 like/unlike/view/sales 4개 연산이 전부 같은 `lastEventAt` 필드 하나를 공유하며 게이팅되고 있었음. view/sales는 단순 누적(교환법칙 성립)이라 순서 자체가 무관한데, 좋아요 이벤트가 먼저 처리돼 `lastEventAt`이 앞서가버리면 그 뒤에 늦게 도착한(하지만 정상적인) 조회/판매 이벤트가 "오래된 이벤트"로 오판돼 부당하게 유실되는 구조였음. `ProductMetricsModel.incrementView()`/`incrementSales()`에서 `eventTime` 파라미터와 `lastEventAt` 갱신을 제거하고, `ProductMetricsService.applyToProductUnordered()`(staleness 체크 없이 항상 반영)를 신설해 view/sales는 이쪽으로, like/unlike는 기존 `applyToProduct()`(staleness 체크)를 그대로 사용하도록 분리 |
